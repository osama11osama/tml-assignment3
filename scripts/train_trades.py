...
Better clean–robust trade-off than pure PGD-AT. Fine-tune from PGD-AT or ERM init.

Local:
  python scripts/train_trades.py --device cuda --init results/checkpoints/pgd_at_resnet18.pt --tag trades_r18

Cluster: docs/CLUSTER_WEEK1.md
"""
...
def parse_args() -> argparse.Namespace:
    default_init = CHECKPOINT_DIR / "pgd_at_resnet18.pt"
    parser = argparse.ArgumentParser(description="TRADES adversarial training")
    ...
    parser.add_argument("--milestones", type=int, nargs="+", default=[25, 35])
    ...
    parser.add_argument("--pgd-train-steps", type=int, default=10)
    parser.add_argument("--pgd-eval-steps", type=int, default=20)
    parser.add_argument("--trades-beta", type=float, default=6.0)
    parser.add_argument("--robust-every", type=int, default=10, help="0 = final epoch only")
    parser.add_argument("--robust-eval-batches", type=int, default=0)
    parser.add_argument("--save-every", type=int, default=5)
    parser.add_argument("--log-every", type=int, default=50)
    parser.add_argument("--num-workers", type=int, default=2)
    parser.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--tag", default="trades_r18")
...
def main() -> None:
    ...
    log_banner("TRADES adversarial training")
    log(f"Run directory: {run_dir}", step="SETUP")
    log(f"Device: {device}", step="SETUP")
    log(
        f"TRADES beta={args.trades_beta:g} | PGD-{args.pgd_train_steps} train | PGD-{args.pgd_eval_steps} eval",
        step="SETUP",
    )
    ...
    if args.resume and last_path.is_file():
        ...
        init_loaded = True
        log(
            f"Resumed at epoch {start_epoch}/{args.epochs} | best unified {best_unified:.4f}",
            step="RESUME",
        )
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        train_loss = train_one_epoch_trades(
            model,
            train_loader,
            optimizer,
            criterion,
