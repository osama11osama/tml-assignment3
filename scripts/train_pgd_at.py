"""
Step 3 — PGD adversarial training (Madry et al.).

Main method for leaderboard robustness. Trains on PGD adversarial examples
with optional init from FGSM-AT (Step 2) or ERM (Step 1).

Features:
  - --init loads a .pt state_dict (default: fgsm_at_resnet18.pt)
  - --resume continues from results/runs/pgd_at_<arch>/last.pt
  - Best checkpoint by validation unified score (PGD-20 eval)

Local:
  python scripts/train_pgd_at.py --device cuda --init results/checkpoints/fgsm_at_resnet18.pt

Cluster (see docs/CLUSTER_STEP3.md):
  bash scripts/cluster/run_pgd_at.sh
  bash scripts/cluster/run_pgd_at.sh --resume
"""
...
def parse_args() -> argparse.Namespace:
    default_init = CHECKPOINT_DIR / "fgsm_at_resnet18.pt"
    parser = argparse.ArgumentParser(description="PGD adversarial training (Step 3)")
    ...
    parser.add_argument("--milestones", type=int, nargs="+", default=[50, 70])
    parser.add_argument("--gamma", type=float, default=0.1)
    parser.add_argument("--val-fraction", type=float, default=0.1)
    parser.add_argument("--pgd-eps", type=float, default=8 / 255)
    parser.add_argument("--pgd-alpha", type=float, default=2 / 255, help="PGD step size (train + eval)")
    parser.add_argument("--pgd-train-steps", type=int, default=7, help="PGD steps during training (7-10 typical)")
    parser.add_argument("--pgd-eval-steps", type=int, default=20, help="PGD steps for validation")
    ...
    parser.add_argument("--tag", default="pgd_at")
    parser.add_argument("--run-dir", type=Path, default=None)
    parser.add_argument(
        "--init",
        type=Path,
        default=default_init if default_init.is_file() else None,
        help="Warm-start state_dict (FGSM-AT or ERM)",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Submit copy path (default: results/checkpoints/pgd_at_<arch>.pt)",
    )
...
def main() -> None:
    ...
    log_banner("STEP 3 — PGD adversarial training (Madry AT)")
    log(f"Run directory: {run_dir}", step="SETUP")
    log(f"Device: {device}", step="SETUP")
    log(
        f"Train PGD-{args.pgd_train_steps} eps={args.pgd_eps:.6f} alpha={args.pgd_alpha:.6f} | "
        f"Eval PGD-{args.pgd_eval_steps}",
        step="SETUP",
    )
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        train_loss = train_one_epoch_pgd(
            model,
            train_loader,
            optimizer,
            criterion,
        ...
        robust_acc = None
        if run_robust:
            log(f"Robust validation (PGD-{args.pgd_eval_steps})...", step="EVAL")
        ...
        if should_save:
            ...
            payload = {
                "method": "pgd_adversarial_training",
    ...
    log(f"Resume with:     python scripts/train_pgd_at.py --resume --device cuda", step="DONE")
