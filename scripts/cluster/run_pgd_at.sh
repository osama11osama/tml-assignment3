#!/usr/bin/env bash
# Run PGD-AT (Step 3) on a GPU node or inside Condor Docker job.
#
# Usage:
#   bash scripts/cluster/run_pgd_at.sh
#   bash scripts/cluster/run_pgd_at.sh --resume
#   TML3_INIT=results/checkpoints/fgsm_at_resnet18.pt bash scripts/cluster/run_pgd_at.sh
#
set -eu
...
LOG="runlogs/pgd_at_${STAMP}.log"

echo "PGD-AT start $(date -Iseconds)" | tee "$LOG"
echo "Python: $PYTHON ($("$PYTHON" --version 2>&1))" | tee -a "$LOG"
echo "Base: $BASE | arch=$ARCH epochs=$EPOCHS batch=$BATCH pgd_train=$PGD_TRAIN_STEPS" | tee -a "$LOG"
echo "Init: $INIT | resume=${RESUME:-no}" | tee -a "$LOG"
...
"$PYTHON" scripts/train_pgd_at.py \
  --device cuda \
  --architecture "$ARCH" \
  --epochs "$EPOCHS" \
  --batch-size "$BATCH" \
